...
public:
    ...
    __host__ __device__ Matrix4f &operator/=(float d);
    ...
    __host__ __device__ Vector4f getRow(int i) const;

    __host__ __device__ void setRow(int i, const Vector4f &v);

    // get column j (mod 4)
    __host__ __device__ Vector4f getCol(int j) const;

    __host__ __device__ void setCol(int j, const Vector4f &v);
    ...
    // starting with upper left corner at (i0, j0)
    __host__ __device__ Matrix3f getSubmatrix3x3(int i0, int j0) const;
    ...
    // starting with upper left corner at (i0, j0)
    __host__ __device__ void setSubmatrix3x3(int i0, int j0, const Matrix3f &m);
    ...
    __host__ __device__ static Matrix4f ones();

    __host__ __device__ static Matrix4f identity();

    __host__ __device__ static Matrix4f translation(float x, float y, float z);

    __host__ __device__ static Matrix4f translation(const Vector3f &rTranslation);

    __host__ __device__ static Matrix4f rotateX(float radians);

    __host__ __device__ static Matrix4f rotateY(float radians);
    ...
    __host__ __device__ static Matrix4f rotation(const Quat4f &q);
    ...
    __host__ __device__ static Matrix4f randomRotation(float u0, float u1, float u2);
