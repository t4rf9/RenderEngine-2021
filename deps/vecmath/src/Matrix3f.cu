...
__host__ __device__ Matrix3f::Matrix3f(float m00, float m01, float m02, float m10,
...
__host__ __device__ Matrix3f::Matrix3f(const Vector3f &v0, const Vector3f &v1,
                                       const Vector3f &v2, bool setColumns) {
    if (setColumns) {
        setCol(0, v0);
        setCol(1, v1);
        setCol(2, v2);
...
__host__ __device__ float &Matrix3f::operator()(int i, int j) {
    return m_elements[j * 3 + i];
}
...
void Matrix3f::print() {
    printf("[ %.4f %.4f %.4f ]\n[ %.4f %.4f %.4f ]\n[ %.4f %.4f %.4f ]\n", m_elements[0],
           m_elements[3], m_elements[6], m_elements[1], m_elements[4], m_elements[7],
           m_elements[2], m_elements[5], m_elements[8]);
}

// static
__host__ __device__ float Matrix3f::determinant3x3(float m00, float m01, float m02,
...
// static
__host__ __device__ Matrix3f Matrix3f::rotation(const Vector3f &rDirection,
                                                float radians) {
