...
__device__ Vector3f BezierCurve::point_at_t(float t) {
    __shared__ extern float shared[];
...
__device__ int BezierCurve::discretize(int resolution, Vector3f **points) {
    int num_points = resolution + 1;
    *points = new Vector3f[num_points];

    float curve_step = 1.f / resolution;
    for (int i = 0; i <= resolution; i++) {
        float t = i * curve_step;
        (*points)[i] = point_at_t(t);
    }
