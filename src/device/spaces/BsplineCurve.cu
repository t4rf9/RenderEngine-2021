...
__device__ Vector3f BsplineCurve::point_at_t(float t) {
    __shared__ extern float shared[];
...
__device__ int BsplineCurve::discretize(int resolution, Vector3f **points) {
    int num_points = (n - k + 1) * (resolution + 1);
