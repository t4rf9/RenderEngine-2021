...
// std::uniform_real_distribution<float> RevSurface::real_0_1_distribution{0, 1};

// std::uniform_real_distribution<float> RevSurface::real_10_distribution{-10, 10};
...
float RevSurface::random01(uint_fast32_t &x) {
...
bool RevSurface::intersect(const Ray &ray, Hit &hit, float t_min, uint_fast32_t &rand) {
    ...
    for (int i = 0; i < repeat_limit; i++) {
        Vector3f x(t_min + random01(rand), random01(rand), 20.f * random01(rand) - 10.f);
        auto &t = x[0];
        auto &u = x[1];
        auto &v = x[2];
